...
const createMessage = (id, conversationId, sender, content, timestamp, senderName, type = 'text', products, buttons) => ({
    id,
    conversationId,
    sender,
    senderName,
...
export const useChatStore = create((set, get) => ({
    ...
    sendMessage: (content) => {
        ...
        set({
            conversations: state.conversations.map(c => c.id === state.selectedConversationId
                ? {
                    ...c,
...
export const useOrderModalStore = create((set) => ({
    ...
    addProduct: (product, size, quantity) => set((state) => ({
