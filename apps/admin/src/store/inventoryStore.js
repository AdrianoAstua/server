...
export const useInventoryStore = create((set, get) => ({
    ...
    setVariantBarcode: (variantId, barcode, format) => {
        const products = get().products.map(p => ({
            ...p,
            variants: p.variants.map(v => v.id === variantId ? { ...v, barcode, barcodeFormat: format || 'CODE128' } : v),
        }));
        set({ products });
    },
...
export function getProductStatus(product) {
    ...
    if (totalStock <= 0)
        return 'out';
    if (hasOut || hasCritical)
        return 'critical';
    const belowMin = stocks.filter((s, i) => s < mins[i]).length;
    if (belowMin > stocks.length * 0.3)
        return 'low';
    return 'ok';
}
export function getVariantStatus(stock, reserved, minStock) {
...
export function formatCRC(cents) {
...
export function getInventoryKPIs(products) {
