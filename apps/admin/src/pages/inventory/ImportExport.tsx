...
import {
  ...
  AlertCircle, Trash2, FileSpreadsheet, Table2
} from 'lucide-react';
...
function exportToExcel(
  products: ReturnType<typeof useInventoryStore.getState>['products'],
  type: 'products' | 'movements' | 'full',
...
function parseUploadedFile(file: File): Promise<PreviewRow[]> {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const preview: PreviewRow[] = rows.slice(0, 50).map((row, i) => {
          // Mapeo flexible de columnas (insensible a mayúsculas/tildes)
          const get = (keys: string[]): string => {
            for (const k of keys) {
              const found = Object.keys(row).find(
                rk => rk.toLowerCase().replace(/[áéíóúü]/g, c => ({ á:'a',é:'e',í:'i',ó:'o',ú:'u',ü:'u' }[c] ?? c))
          ...
          if (!talla)   errors.push('Talla requerida');
...
function ExportSection() {
  ...
  const [exportType, setExportType] = useState<'products' | 'movements' | 'full'>('products');
    ...
    <div className="bg-[#111111] border border-[#1E1E1E] rounded-xl p-5">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-[#6B7280]">Descarga inventario y movimientos en Excel</p>
        </div>
      </div>

      <div className="grid grid-cols-3 gap-2 mb-4">
        {(['products', 'movements', 'full'] as const).map(type => (
          <button
            key={type}
            onClick={() => setExportType(type)}
            className={`py-2.5 px-3 rounded-lg text-xs font-medium transition-colors border ${
              exportType === type
                ? 'bg-[#00FF88]/10 text-[#00FF88] border-[#00FF88]/20'
                : 'text-[#6B7280] border-[#2A2A2A] hover:text-white'
            }`}
          >
            {type === 'products' ? '📦 Productos' : type === 'movements' ? '🔄 Movimientos' : '📊 Completo'}
          </button>
        ))}
      </div>

      <div className="bg-[#1A1A1A] rounded-lg px-4 py-3 mb-4 flex items-center justify-between">
        <div>
          <p className="text-xs text-[#6B7280]">Filas a exportar</p>
          <p className="text-lg font-bold text-white">{rowCount}</p>
        </div>
        <div className="text-right">
          <p className="text-xs text-[#6B7280]">Formato</p>
          <p className="text-sm font-semibold text-white">Excel (.xlsx)</p>
        </div>
      </div>

      <button
        onClick={() => exportToExcel(products, exportType, movements)}
        className="w-full py-3 rounded-xl bg-[#00FF88] text-black font-bold text-sm hover:bg-[#00DD77] transition-colors flex items-center justify-center gap-2"
      >
  ...
  return (
    <div className="bg-[#111111] border border-[#1E1E1E] rounded-xl p-5">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="text-sm font-bold text-white">Importar productos</h3>
          <p className="text-xs text-[#6B7280]">Sube un archivo Excel o CSV con los productos</p>
        </div>
      </div>
      ...
      {!fileName ? (
        ...
        >
          {loading ? (
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="text-sm text-[#6B7280]">Procesando archivo...</p>
            </div>
          ) : (
            <>
              <FileSpreadsheet className="w-9 h-9 text-[#4B5563] mx-auto mb-3" />
              ...
              <div className="flex items-center justify-center gap-3 mt-3">
                <span className="text-xs bg-[#1A1A1A] border border-[#2A2A2A] px-2 py-1 rounded text-[#6B7280]">.xlsx</span>
                <span className="text-xs bg-[#1A1A1A] border border-[#2A2A2A] px-2 py-1 rounded text-[#6B7280]">.xls</span>
                <span className="text-xs bg-[#1A1A1A] border border-[#2A2A2A] px-2 py-1 rounded text-[#6B7280]">.csv</span>
              </div>
            </>
          )}
          <input
        ...
        <>
          {/* File info + stats */}
          <div className="bg-[#1A1A1A] rounded-lg px-4 py-3 mb-3 flex items-center gap-3">
            <FileSpreadsheet className="w-5 h-5 text-[#3B82F6] flex-shrink-0" />
            <div className="flex-1 min-w-0">
              <p className="text-sm text-white font-medium truncate">{fileName}</p>
              <p className="text-xs text-[#6B7280]">{preview.length} filas detectadas</p>
            </div>
            <button onClick={reset} className="p-1.5 text-[#6B7280] hover:text-white">
          ...
          <div className="flex items-center gap-3 mb-3">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-sm text-[#00FF88] font-semibold">{validRows} válidas</span>
            </div>
            {invalidRows > 0 && (
              <div className="flex items-center gap-1.5">
                <XCircle className="w-4 h-4 text-[#EF4444]" />
                <span className="text-sm text-[#EF4444] font-semibold">{invalidRows} con errores</span>
              </div>
            )}
          </div>

          {/* Preview table */}
          <div className="bg-[#0F0F0F] border border-[#1E1E1E] rounded-lg overflow-hidden mb-4">
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-[#1E1E1E] text-[#6B7280]">
                ...
                <tbody>
                  {preview.map(row => (
                    <tr
                      key={row.row}
                      className={`border-b border-[#111111] ${row.valid ? '' : 'bg-[#EF4444]/5'}`}
                      title={row.errors.join(' · ')}
                    >
                      <td className="py-2 px-3 text-[#4B5563]">{row.row}</td>
                      <td className="py-2 px-3 font-mono text-[#9CA3AF]">{row.sku || '—'}</td>
                      <td className="py-2 px-3 text-white max-w-[160px] truncate">{row.nombre || '—'}</td>
                      <td className="py-2 px-3 text-[#9CA3AF]">{row.talla || '—'}</td>
                      <td className="py-2 px-3 text-[#9CA3AF]">{row.color || '—'}</td>
                      <td className="py-2 px-3 text-right text-white">
                        {row.precio > 0 ? `₡${row.precio.toLocaleString('es-CR')}` : '—'}
                      </td>
                      <td className="py-2 px-3 text-right font-semibold text-white">{row.stock}</td>
                      <td className="py-2 px-3 text-center">
                        {row.valid
                          ? <CheckCircle2 className="w-3.5 h-3.5 text-[#00FF88] mx-auto" />
                          : (
                            <span title={row.errors.join(', ')}>
                              <XCircle className="w-3.5 h-3.5 text-[#EF4444] mx-auto" />
                            </span>
                          )
                        }
                      </td>
          ...
          {imported ? (
            <div className="flex items-center gap-2 justify-center py-3 text-[#00FF88]">
              <CheckCircle2 className="w-5 h-5" />
              <span className="text-sm font-semibold">¡{validRows} productos importados!</span>
            </div>
          ) : (
            <button
              onClick={() => setImported(true)}
              disabled={validRows === 0}
              className="w-full py-3 rounded-xl bg-[#3B82F6] text-white font-bold text-sm hover:bg-[#2563EB] transition-colors flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed"
            >
              <Upload className="w-4 h-4" />
              Importar {validRows} producto{validRows !== 1 ? 's' : ''}
            </button>
          )}
        </>
      )}
      ...
      <button
        ...
        className="w-full mt-3 py-2.5 rounded-lg border border-[#2A2A2A] text-[#6B7280] hover:text-white hover:border-[#3B82F6]/40 text-xs transition-colors flex items-center justify-center gap-2"
      >
    ...
    <div className="bg-[#111111] border border-[#1E1E1E] rounded-xl p-5">
      <div className="flex items-center gap-3 mb-4">
        <div className="p-2 rounded-lg bg-[#F59E0B]/10">
          <Table2 className="w-5 h-5 text-[#F59E0B]" />
        </div>
        <div>
          <h3 className="text-sm font-bold text-white">Estructura de la plantilla</h3>
          <p className="text-xs text-[#6B7280]">Columnas requeridas para importar productos</p>
        </div>
        <button
          onClick={downloadTemplate}
          className="ml-auto flex items-center gap-1.5 text-xs text-[#00FF88] hover:underline"
        >
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        {TEMPLATE_COLUMNS.map(col => (
          ...
          >
            <span className="font-bold">{col.key}</span>
            {col.required && (
              <span className="text-[#00FF88]/50 text-[10px]">*</span>
            )}
          </div>
        ))}
      </div>
              ...
              {TEMPLATE_COLUMNS.map(col => (
                <th
                  ...
                  className={`text-left py-2 px-3 font-semibold whitespace-nowrap ${
                    col.required ? 'text-[#00FF88]' : 'text-[#6B7280]'
                  }`}
                >
                  {col.key}
                  {col.required && <span className="text-[#EF4444] ml-0.5">*</span>}
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {EXAMPLE_ROWS.slice(0, 3).map((row, i) => (
              <tr key={i} className="border-b border-[#111111] hover:bg-[#111111]">
                {row.map((cell, j) => (
                  <td key={j} className="py-2 px-3 text-[#9CA3AF] whitespace-nowrap">
                    {typeof cell === 'number' && j === 3
            ...
            <tr>
              <td colSpan={TEMPLATE_COLUMNS.length} className="py-2 px-3 text-center text-[#4B5563] text-xs italic">
                … continúa con el resto de tus productos
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div className="mt-3 flex items-center gap-4 text-xs text-[#6B7280]">
        <span><span className="text-[#00FF88] font-semibold">*</span> Columna requerida</span>
        <span>· Precio y Costo en colones (₡), sin decimales</span>
        <span>· Stock mínimo sugerido: 3</span>
      </div>
    </div>
  );
}
...
export function InventoryImportExport() {
  return (
    <div className="h-full overflow-y-auto bg-[#0A0A0A] p-5 space-y-4">
      <div>
        <h1 className="text-xl font-bold text-white">Importar / Exportar</h1>
        <p className="text-sm text-[#6B7280] mt-0.5">Gestiona datos de inventario con Excel</p>
      </div>

      {/* Export + Import en fila */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <ExportSection />
        <ImportSection />
      </div>

      {/* Plantilla visual abajo */}
