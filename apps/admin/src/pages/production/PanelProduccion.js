...
import { ScanLine, Camera, X, Video, AlertTriangle, CheckCircle, Clock, Printer, Scissors, Hammer, Package, BarChart3, } from 'lucide-react';
    ...
    useEffect(() => {
        ...
        async function startScanner() {
            try {
                ...
                await scanner.start({ facingMode: 'environment' }, { fps: 10, qrbox: { width: 280, height: 160 }, aspectRatio: 16 / 9, disableFlip: false }, (decodedText) => {
...
export default function PanelProduccion() {
    ...
    const currentStation = activeStation ?? 'IMPRESION';
    ...
    const stationQueue = useMemo(() => MOCK_QUEUE.filter((q) => q.station === currentStation)
        .sort((a, b) => {
    ...
    const handleCameraDetect = useCallback((code) => handleScan(code), [handleScan]);
    // ─── Render ─────────────────────────────────────────────────────────────
    const stations = ['IMPRESION', 'CORTE', 'ARMADO'];
    return (_jsxs("div", { className: "h-full flex flex-col bg-[#0A0A0A]", children: [cameraOpen && _jsx(CameraScanner, { onDetect: handleCameraDetect, onClose: () => setCameraOpen(false) }), _jsx("div", { className: "flex-shrink-0 flex border-b border-[#1E1E1E]", children: stations.map((st) => {
                    const cfg = STATION_CONFIG[st];
                    const Icon = cfg.icon;
                    const isActive = currentStation === st;
                    const count = MOCK_QUEUE.filter((q) => q.station === st).length;
                    return (_jsxs("button", { onClick: () => setActiveStation(st), className: `flex-1 flex items-center justify-center gap-3 py-5 px-4 text-base font-semibold transition-all border-b-2 ${isActive
                            ? 'border-b-current text-white bg-[#111111]'
                            : 'border-b-transparent text-[#6B7280] hover:text-white hover:bg-[#0F0F0F]'}`, style: isActive ? { color: cfg.color } : undefined, children: [_jsx(Icon, { className: "w-6 h-6" }), _jsx("span", { children: cfg.label.toUpperCase() }), _jsx("span", { className: "text-xs px-2.5 py-1 rounded-full font-bold", style: isActive ? { backgroundColor: `${cfg.color}20`, color: cfg.color } : { backgroundColor: '#1E1E1E', color: '#6B7280' }, children: count })] }, st));
                }) }), _jsxs("div", { className: "flex-shrink-0 px-6 py-5 border-b border-[#1E1E1E]", children: [_jsxs("div", { className: "flex gap-3", children: [_jsxs("div", { className: "relative flex-1", children: [_jsx(ScanLine, { className: "absolute left-5 top-1/2 -translate-y-1/2 w-6 h-6 text-[#6B7280]" }), _jsx("input", { ref: inputRef, value: scanValue, onChange: (e) => setScanValue(e.target.value), onKeyDown: handleKeyDown, placeholder: "Escanear codigo de barras de la pieza...", autoFocus: true, className: "w-full pl-14 pr-6 py-5 rounded-2xl text-xl font-mono border-2 border-[#1E1E1E] bg-[#0F0F0F] text-white placeholder:text-[#4B5563] focus:outline-none focus:border-[#00FF88]/50 transition-colors" })] }), _jsx("button", { onClick: () => setCameraOpen(true), className: "flex-shrink-0 w-20 rounded-2xl border-2 border-[#2A2A2A] bg-[#111111] flex items-center justify-center text-[#9CA3AF] hover:text-white hover:border-[#3A3A3A] transition-all active:scale-95", children: _jsx(Camera, { className: "w-7 h-7" }) })] }), lastScan && (_jsxs("div", { className: `mt-4 flex items-center gap-3 px-5 py-4 rounded-xl border ${lastScan.type === 'success'
                            ? 'bg-[#00FF88]/5 border-[#00FF88]/20'
                            : 'bg-red-500/5 border-red-500/20'}`, children: [lastScan.type === 'success' ? (_jsx(CheckCircle, { className: "w-6 h-6 text-[#00FF88] flex-shrink-0" })) : (_jsx(AlertTriangle, { className: "w-6 h-6 text-red-400 flex-shrink-0" })), _jsxs("div", { children: [_jsx("p", { className: `text-sm font-medium ${lastScan.type === 'success' ? 'text-[#00FF88]' : 'text-red-400'}`, children: lastScan.message }), lastScan.detail && (_jsx("p", { className: "text-xs text-[#9CA3AF] mt-0.5", children: lastScan.detail }))] })] }))] }), _jsxs("div", { className: "flex-1 overflow-y-auto px-6 py-4", children: [_jsxs("h2", { className: "text-xs font-semibold text-[#6B7280] uppercase tracking-wider mb-3", children: ["Cola de ", STATION_CONFIG[currentStation].label, " (", stationQueue.length, " piezas)"] }), stationQueue.length === 0 ? (_jsxs("div", { className: "flex flex-col items-center justify-center py-16 text-[#6B7280]", children: [_jsx(Package, { className: "w-12 h-12 mb-4 opacity-20" }), _jsx("p", { className: "text-sm font-medium text-[#9CA3AF]", children: "Estacion vacia" }), _jsx("p", { className: "text-xs mt-1", children: "No hay piezas esperando en esta estacion" })] })) : (_jsx("div", { className: "space-y-2", children: stationQueue.map((item) => {
                            const priBorder = PRIORITY_BORDER[item.priority];
                            const priBadge = PRIORITY_BADGE[item.priority];
                            return (_jsxs("div", { className: `flex items-center gap-4 bg-[#111111] border border-[#1E1E1E] rounded-xl px-5 py-4 border-l-2 ${priBorder} hover:border-[#2A2A2A] transition-colors`, children: [_jsxs("div", { className: "flex-shrink-0", children: [_jsx("p", { className: "text-xs font-mono text-[#00FF88]", children: item.barcode }), _jsx("p", { className: "text-[10px] text-[#6B7280] mt-0.5", children: item.workOrderNumber })] }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsx("p", { className: "text-sm text-white font-medium truncate", children: item.productType }), _jsx("p", { className: "text-xs text-[#9CA3AF] truncate", children: item.description })] }), _jsx("span", { className: `text-[10px] font-semibold px-2.5 py-1 rounded-full ${priBadge.bg} ${priBadge.text}`, children: item.priority }), _jsxs("span", { className: "text-xs text-[#6B7280] flex items-center gap-1 flex-shrink-0 min-w-[60px] justify-end", children: [_jsx(Clock, { className: "w-3.5 h-3.5" }), timeWaiting(item.enteredAt)] })] }, item.id));
                        }) }))] }), _jsx("div", { className: "flex-shrink-0 border-t border-[#1E1E1E] px-6 py-4", children: _jsxs("div", { className: "flex items-center justify-around", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx(Package, { className: "w-5 h-5 text-[#3B82F6]" }), _jsxs("div", { children: [_jsx("p", { className: "text-lg font-bold text-white", children: stationQueue.length }), _jsx("p", { className: "text-[10px] text-[#6B7280]", children: "En esta estacion" })] })] }), _jsx("div", { className: "w-px h-10 bg-[#1E1E1E]" }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsx(ScanLine, { className: "w-5 h-5 text-[#00FF88]" }), _jsxs("div", { children: [_jsx("p", { className: "text-lg font-bold text-[#00FF88]", children: scannedToday }), _jsx("p", { className: "text-[10px] text-[#6B7280]", children: "Escaneadas hoy" })] })] }), _jsx("div", { className: "w-px h-10 bg-[#1E1E1E]" }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsx(BarChart3, { className: "w-5 h-5 text-[#F59E0B]" }), _jsxs("div", { children: [_jsxs("p", { className: "text-lg font-bold text-white", children: [avgTime, " min"] }), _jsx("p", { className: "text-[10px] text-[#6B7280]", children: "Tiempo promedio" })] })] })] }) })] }));
}
