...
import type { ConversationEvent, NotificationEvent } from '@/types/api';
import { conversationKeys } from './use-conversations';
import { notificationKeys } from './use-notifications';

type ConversationEventHandler = (event: ConversationEvent) => void;
 ...
 */
export function useConversationEvents(onEvent?: ConversationEventHandler) {
  ...
  useEffect(() => {
    ...
    es.onmessage = (messageEvent) => {
      try {
        const data = JSON.parse(messageEvent.data as string) as ConversationEvent;
        handleEvent(data);
      } catch {
 ...
 * @param onEvent - Optional callback for custom handling of each event.
 */
export function useNotificationEvents(onEvent?: NotificationEventHandler) {
  const queryClient = useQueryClient();
  const isAuthenticated = useAuthStore((s) => s.isAuthenticated);
  const eventSourceRef = useRef<EventSource | null>(null);
  const onEventRef = useRef(onEvent);
  ...
  useEffect(() => {
    ...
    es.onmessage = (messageEvent) => {
      try {
        const data = JSON.parse(messageEvent.data as string) as NotificationEvent;
        handleEvent(data);
      } catch {
        // Ignore malformed events
      }
